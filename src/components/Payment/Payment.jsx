...
export default function Payment() {
  ...
  const {cartPrice,cartId,ClearCart,GetCartItems} = useContext(cartContext);
...
const cashPayment = async () => {
  try {
    const response = await axios.post(
      `https://ecommerce.routemisr.com/api/v1/orders/${cartId}`, addressDetails,
      {
        headers: {
          token: token,
        },
    ...
    GetCartItems(); // Refresh the cart items after clearing
    navigate('/')
...
const handlePaymentMethodChange = (method) => {
  setPaymentMethod(method); // Update payment method state  
  // Show payment method selection UI if needed
...
const creditPayment = async () => {
  try {
    const response = await axios.post(
      `https://ecommerce.routemisr.com/api/v1/orders/checkout-session/${cartId}`,addressDetails,
      {
        headers: {
          token: token,
        },
    ...
    // Handle error
    console.error("Error placing Credit order", error);
    // Show error message to the user
  }
};
...
if (paymentMethod === 'cash') {
  cashPayment(); // Call cash payment function when payment method is cash
} else {
  creditPayment(); // Call credit payment function when payment method is credit
}
}
          ...
          <div className="mx-auto max-w-5xl">
            ...
            <div className="mt-6 sm:mt-8 lg:flex lg:items-start lg:gap-8">
              ...
              <div className="lg:w-1/2 lg:pr-4">
                ...
                {showSavedAddresses ? (
                  <div className="w-full rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800 sm:p-6 lg:p-8 mb-6">
              ...
              <div className="mt-6 grow sm:mt-8 lg:mt-0 border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
